...
with st.expander("📖 트랜센던트 레조넌스 시스템 매뉴얼 (v4.8.2)", expanded=False):
    st.markdown("""
    ### 🌌 시스템 핵심 원리
    본 시스템은 Gemini의 도움을 받아 미루던 python을 일주일만에 공부해가며 만든 프로그램으로
    **천·지·인(天·地·人) (삼재)**의 에너지를 20:35의 임계점에서 공명시킵니다.
    
    1. **地 (Earth - Ace Line):** `master_list.xlsm`의 최근 55회차 데이터를 분석하여 통계적 가중치를 부여합니다. 
            (땅의 일이자 당신 환경의 일이며, 과거의 축적)
    2. **天 (Sky - Celestial):** 분석 당일 **20:35**의 행성 배치와 각도를 계산하여 우주의 기운을 수치화합니다. 
            (하늘의 일이자 현재의 흐름)
    3. **人 (Human - Destiny):** 사용자의 고유 에너지와 미래 당첨 시점(**토요일 20:35**) 사이의 공명각을 추출합니다. 
            (하늘과 땅 사이에서 조화를 이루는 당신의 일이자, 미래의 확정)

    *분석일에 생일을 입력하시면 공명카드에 네이탈차트를 만들 수 있습니다만... 
    시각을 20:35로 고정했기에 정확한 생시반영은 안됩니다.
    *공명카드는 이미지 다운로드가 안됩니다. 그냥 캡쳐하세요. 죄송합니다. 
    ### 🛠️ 운영 가이드
    * **데이터 업데이트:** 매주 추첨 결과 발표 후 엑셀 파일을 깃허브에 재업로드하겠습니다..만 잊어버릴 수도 있습니다.
    * **분석 시점:** 가급적 마음이 평온한 상태에서 **20:35** 근처에 분석을 실행하는 것을 권장합니다. 시간을 고정해버려서요.^^;; 
    * **기록 보존:** 하단의 '🚀 전체 16개 세트 기록' 버튼을 눌러 공명 데이터를 아카이브 안 하셔도 됩니다.
                  개인적으로 저에겐 도움이 됩니다. 모인 데이터들을 제가 분석해보고 학습시켜볼 계획입니다만, 
                  그것 역시 바쁘면 못할 수도 있습니다.. 본명 안쓰셔도 돼요.
...
# 1. 생년월일 입력
    birthday = st.date_input(
        "생년월일", 
        date(1990, 1, 1),
        min_value=date(1800, 1, 1), # 성인 분석을 위해 확장
        max_value=date(2100, 12, 31)
    )
    
    # 2. 날짜 선택 (d_input으로 임시 저장)
